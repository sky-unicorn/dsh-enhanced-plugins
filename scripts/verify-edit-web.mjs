...
const home = mkdtempSync(resolve(scratch, 'edit-web-610-'))
...
try {
  ...
  await composer.fill('ORIGINAL_COMPAT_PROMPT')
  ...
  for (const [index, scheme] of ['light', 'dark'].entries()) {
    ...
    await editor.fill(`REVISED_COMPAT_${index + 1}`)
    await page.screenshot({ path: resolve(home, `editor-${scheme}.png`), fullPage: true })
    ...
    await page.locator('[data-chat-flow-key]').getByText('ORIGINAL_COMPAT_PROMPT', { exact: true }).waitFor({ state: 'hidden' })
    ...
    await page.screenshot({ path: resolve(home, `reloaded-${scheme}.png`), fullPage: true })
  }
  ...
  for (const scheme of ['dark', 'light']) {
    await page.emulateMedia({ colorScheme: scheme })
    await page.waitForFunction(dark => document.body.hasAttribute('data-ds-dark-theme') === dark, scheme === 'dark')
    await page.screenshot({ path: resolve(home, `attachment-${scheme}.png`), fullPage: true })
  }
  assert.deepEqual(errors, [])
  writeFileSync(resolve(home, 'report.json'), JSON.stringify({ requests: requests.length, repeatedEdits: true, reload: true, attachment: true, schemes: ['light', 'dark'], pageErrors: errors }, null, 2))
