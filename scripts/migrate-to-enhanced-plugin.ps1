...
function New-ProfileBundleArchives {
  param(
    [Parameter(Mandatory = $true)][object[]] $Packages,
    [Parameter(Mandatory = $true)][string] $ProfileName
  )

  # Runtime resolution owns package dependencies inside the profile. A link to
  # an external checkout leaves peer imports outside that resolver's scope.
  # Retain immutable archives because pnpm's file: entries need them on reinstall.
  ...
  $cacheRoot = [System.IO.Path]::GetFullPath((Join-Path (Join-Path (Join-Path $dshHome 'profiles') $ProfileName) '.dsh-enhanced-bundles'))
  ...
  try {
    foreach ($package in $Packages) {
      Write-Host "Packing '$($package.PackageName)' for profile-local installation..."
      $packOutput = @(& $npm.Source pack --ignore-scripts --json --pack-destination $staging $package.Root) -join [Environment]::NewLine
      if ($LASTEXITCODE -ne 0) { throw "Cannot pack '$($package.PackageName)'; installed bundles were not changed." }
      $packed = @($packOutput | ConvertFrom-Json)
      if ($packed.Count -ne 1 -or $packed[0].name -cne $package.PackageName) { throw 'npm pack returned an unexpected package.' }
      $filename = [string]$packed[0].filename
      if ([System.IO.Path]::GetFileName($filename) -cne $filename -or -not $filename.EndsWith('.tgz')) {
        throw 'npm pack returned an invalid archive path.'
      }
      $archive = Join-Path $staging $filename
      $hash = Get-FileSha256 -Path $archive
      $target = Join-Path $cacheRoot ($package.PackageName + '-' + $hash + '.tgz')
      if (Test-Path -LiteralPath $target -PathType Leaf) {
        if ((Get-FileSha256 -Path $target) -cne $hash) { throw 'Cached bundle archive failed its content hash check.' }
      } else {
        Move-Item -LiteralPath $archive -Destination $target
      }
      $target
    }
  } finally {
    $absoluteStaging = [System.IO.Path]::GetFullPath($staging)
    if (-not $absoluteStaging.StartsWith($cacheRoot + '\', [System.StringComparison]::OrdinalIgnoreCase)) {
      throw 'Refusing archive cleanup outside the profile bundle cache.'
    }
...
function Set-WindowsLauncherDshCommand {
  ...
  if ($PrefixArguments.Count -gt 0 -or -not [string]::IsNullOrWhiteSpace($RunnerWorkingDirectory)) {
    ...
    $cliEntry = Join-Path $checkout 'apps\cli\lib\bin.js'
    ...
    if (-not (Test-Path -LiteralPath $cliEntry -PathType Leaf)) {
      throw "The DSH checkout at '$checkout' is missing apps/cli/lib/bin.js. Build DSH before installing the Launcher."
    }
    ...
    $invokerContent = @(
      ...
      "if (-not (Test-Path -LiteralPath $cliLiteral -PathType Leaf)) { throw 'DSH CLI build is missing. Build DSH before starting it.' }"
      "& $nodeLiteral $cliLiteral @args"
      'exit $LASTEXITCODE'
