...
const cliArgs = ['--import', pathToFileURL(resolve(dsh, 'node_modules/tsx/dist/esm/index.mjs')).href,
  resolve(dsh, 'apps/cli/src/bin.ts')]
...
try {
  ...
  for (const scheme of ['light', 'dark']) {
    ...
    if (scheme === 'light') {
      ...
      await panel.getByRole('textbox', { name: 'Server name', exact: true }).fill('fixture-server')
      await panel.getByRole('textbox', { name: 'Command', exact: true }).fill(process.execPath)
      await panel.getByRole('textbox', { name: 'Argument 1', exact: true }).fill(mcpFixture)
