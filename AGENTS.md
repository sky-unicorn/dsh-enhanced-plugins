...
- `agent-team-monitor`（执行过程监控）与 `model-router`（多模型协作）当前仅兼容 DSH `0.1.7-alpha.1`，并以 Beta 功能发布。
- 后续插件版本或兼容表更新时，除非用户明确提出要为这两个功能进行版本兼容，否则不得为它们新增、放宽或迁移 DSH 兼容版本；其它功能按正常兼容流程处理。
