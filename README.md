...
- This repository is verified against DSH [`0.1.2-alpha.4`](https://github.com/deepseek-ai/deepseek-harness/tree/4e84901e6471b79ec0338099867ebb4606d12bb5), with local ABI baseline commit `4e84901e6471b79ec0338099867ebb4606d12bb5`.
...
- **Version pairing:** plugin `0.5.0` targets DSH `0.1.2-alpha.4`, source commit `4e84901e6471b79ec0338099867ebb4606d12bb5`; its release tag is `0.5.0/dsh-0.1.2-alpha.4`. The aggregate, all seven standalone bundles, and Windows Launcher use `0.5.0`.
...
- **Current interfaces:** Client features use `client-store`, `ui-session`, `ui-chat`, and public Remotes, without the removed `dsh-client-runtime`, `connection.api`, or `hostDescription`. Host settings owners pass validated namespace literals and use `SettingsProvider.installSection()`. Session consumers use alpha.4's `eventAt()` / `snapshotEvents()` APIs and keep `SessionLogOffset` inheritance metadata separate from `SessionHeader`; Team Monitor passes that exact cut through persistence inspection and projection replay. This project follows the public interfaces of the source commit above.
