...
it('consumes semantic tokens without pinning a color or branching on the Host theme', () => {
  ...
  const css = ['Panel.module.css', 'RoleSessions.module.css', 'ExecutionFlow.module.css', 'CooperationFlow.module.css'].map(file => readFileSync(resolve(root, file), 'utf8')).join('\n')
  ...
  for (const file of ['Panel.tsx', 'RoleSessions.tsx', 'ExecutionFlow.tsx']) expect(readFileSync(resolve(root, file), 'utf8')).not.toMatch(/matchMedia|getComputedStyle/)
})
