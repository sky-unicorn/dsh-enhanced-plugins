...
describe('selective feature packages', () => {
  ...
  it('keeps the Windows companion outside Cordis and avoids port-owner termination', () => {
    ...
    const terminalText = readFileSync(resolve(launcherRoot, 'src', 'TerminalText.cs'), 'utf8')
    ...
    expect(program).toContain('TerminalText.SelfTest()')
    ...
    expect(program).toContain('NewButton("更新源码并构建"')
    expect(program).toContain('NewButton("仅构建"')
    expect(program).toContain('buildDshButton.Click += delegate { RunDshBuild(true); }')
    expect(program).toContain('buildDshOnlyButton.Click += delegate { RunDshBuild(false); }')
    expect(program).toContain('MessageBox.Show(this, confirmation, requestSourceUpdate ? "更新并构建 DSH" : "仅构建 DSH"')
    expect(program).toContain('bool updateSource = requestSourceUpdate && runtime.IsGitAvailable()')
    ...
    expect(runtime).toContain('output = TerminalText.ForDisplay(stdout')
    expect(runtime).toContain('return TerminalText.ForDisplay(String.Join(Environment.NewLine, tail.ToArray()))')
    expect(pluginUi).toContain('string tail = TerminalText.ForDisplay(')
    expect(terminalText).toContain("if (current == '\\u009B')")
    expect(terminalText).toContain('internal static bool SelfTest()')
    ...
    expect(supervisor).toContain("$ErrorActionPreference = 'Continue'")
    expect(supervisor).toContain('$taskkillExitCode = $LASTEXITCODE')
    expect(supervisor).toContain('if ($runner.HasExited)')
