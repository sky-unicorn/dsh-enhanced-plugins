...
describe('0.2.0 release compatibility', () => {
  it('keeps the aggregate, standalone packages, native version and DSH peers aligned', () => {
    ...
    expect(release.dshEnhanced.compatibility).toEqual({
      dshVersion: '0.1.2-alpha.1', sourceCommit: 'cd5ef8148158c3a752a658978873241fdf8e2bbc',
    })
  ...
  it.runIf(process.platform === 'win32')('accepts a matching source ZIP, warns about unverifiable Git, and writes nothing', () => {
    ...
    expect(result.output).toContain('Compatibility OK: plugin 0.2.0 -> DSH 0.1.2-alpha.1')
  ...
  it.runIf(process.platform === 'win32').each(['0.1.1-rc.2', '0.1.2-alpha.2', '0.1.2'])('rejects DSH %s before build or installation', (version) => {
  ...
  it.runIf(process.platform === 'win32')('rejects missing compatibility metadata and an unrelated checkout identity', () => {
    ...
    writeFileSync(resolve(source.dsh, 'package.json'), JSON.stringify({ name: 'unrelated', version: '0.1.2-alpha.1' }))
