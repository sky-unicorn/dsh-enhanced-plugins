...
import { Config, snapshotNotificationSettings } from '../../src/notification/host/config.ts'
import {
...
describe('desktop notification assets and defaults', () => {
  it('resolves the complete schema defaults', () => {
    expect(snapshotNotificationSettings(Config({}))).toEqual(DEFAULT_NOTIFICATION_SETTINGS)
    expect(snapshotNotificationSettings(Config({ petCharacter: 'whale-girl' })).petCharacter).toBe('whale-girl')
    expect(snapshotNotificationSettings(Config({ petCharacter: 'beijing-shark' } as never)).petCharacter).toBe('classic')
