...
export function snapshotRouterConfig(value: unknown): RouterConfig {
  ...
  return visit(value) as RouterConfig
}
...
export const Config = Schema.object({
  ...
  models: Schema.object({ light: model, normal: model, strong: model }).volatile(),
  ...
  budget: Schema.object({ tokenLimitEnabled: Schema.boolean().default(false), maxTotalTokens: Schema.natural().min(1).max(10000000).default(200000) }).volatile(),
  ...
  })).max(100).default([]).volatile(),
  retry: Schema.object({ maxRetries: Schema.natural().max(3).default(2) }).volatile(),
  ...
  storage: Schema.object({ maxRunRecordBytes: Schema.natural().min(65536).max(16777216).default(2097152) }).volatile(),
})
