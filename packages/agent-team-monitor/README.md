...
Read-only Host adapter and Web panel for DSH `0.1.1-rc.2` (ABI `b150a551`).
...
Live members come from `ctx.agentTeams`; historical roster/tasks/mailbox counts are replayed by the official public `foldTeam` export, resolved from the running profile.
...
实时成员来自官方服务，历史通过官方公开的 `foldTeam` 从 Session 日志回放。
