...
const externals = ['react', 'react/jsx-runtime', 'react-dom', 'react-dom/client', '@deepseek-ai/dsh-client-store', '@deepseek-ai/dsh-client-ui-primitives']

/** Self-contained prepare: source checkouts use the shared source; tarballs include a buildable snapshot of it. */
export async function build() {
  const shared = resolve(root, '../../src/model-router')
  ...
  await esbuild.build({ entryPoints: [resolve(source, 'host/index.ts')], outfile: resolve(lib, 'model-router/host/index.js'),
  ...
  if (js === undefined) throw new Error('Model router client bundle missing')
