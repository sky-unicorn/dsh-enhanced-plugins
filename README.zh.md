...
| [插件社区](#3-插件社区) | `plugin-market` | `dsh-enhanced-plugin-market` | Web；设置 → 插件社区 | 浏览社区插件，通过 DSH 原生管理器安装 |
...
![执行流程与节点详情](assets/readme/agent-team-monitor.png)

- 主视图是协作总览：主智能体派发任务，各队员展示“派发 → 执行 → 回传 → 接收”阶段、当前操作、步骤／工具完成数及耗时，返回连线汇合到实际父会话；嵌套派发独立分组。点击队员进入内部执行泳道，点击派发、进度消息或回调查看对应记录。内部流程默认展示步骤，开启“显示工具与轮次”可展开调用记录；可继续打开原生会话。
- 步骤／工具数量是最新轮次已记录的进度，不是预估任务总量。回调只认实际工作流结束事件或父会话收到的完成通知；普通进度消息不算完成，父会话收到也不代表验收通过。原生一次性队员可能只有执行结束记录、没有持久化回调，此时明确显示“无回传记录”，不会补造成功连线。协作事件最多保留最近 1,000 条并提示截断。
- 箭头表示记录的开始顺序，不推测因果或依赖，也不是按时间比例排列的甘特图。父子入口来自原生目录；工作流中实际记录的派发节点可进入对应队员。并行调用分别保留，可对照开始／结束时间查看重叠。暂不生成尚未执行的计划节点或自动归纳业务阶段。
- 每条泳道最多展示 120 个节点，优先保留执行中节点和最近记录，并明确提示截断；更早记录可进入原生会话查看。详情按需读取，每个输入／输出字段最多 6,000 字符，常见凭据字段做展示脱敏。只展示公开文本与工具参数／结果，不额外读取 reasoning、系统提示、私有工具 meta 或 provider 异常原文。没有文本的节点显示空记录提示。
- 入口属于当前会话，在输入框右侧的模型／上下文控件同组显示。检测到执行记录、工作流、Agent Teams 或原生子代理会话后出现图标；点击才展开，切换会话立即关闭并清除旧数据。普通单智能体会话也支持；无需启用官方团队插件。原成员分组、任务板和工作流汇总作为辅助信息保留。
...
- 点击可用会话行，会按真实父子 ID 和原生目录模式调用 DSH 的 `uiWorkspace.openSession()`，进入原生会话详情。历史、正在执行、嵌套会话均可跳转；点击前重新核对目录，切换会话或插件卸载后不执行迟到跳转。损坏／缺失的记录显示不可用，不伪造空会话。
...
- 使用兼容表中已验证的 DSH 源码 ABI。实验包不由本插件打包或从 npm 获取；Agent Teams 历史回放依赖运行 profile 中已挂载的官方运行时及其投影。成员运行状态不等于任务完成状态：“未驻留”不是“已完成”，任务板状态仍依赖模型更新；流程状态来自实际执行日志。团队汇总不传输邮箱正文。最多显示 256 个成员 / 1,000 个任务，超限时明确提示，汇总仍包含全部记录。
- 工作流最多显示 100 次运行、合计 256 条成员记录，汇总保持完整。未记录结束的冷历史不会冒充实时运行；原步骤或轮次已关闭时标记中断。监控只读取公开记录，不读取／执行工作流脚本，普通子代理不会被伪装成实验性 Team。
- 原生子会话目录来自公开的 `subagents.listDescendants`，通过只读的 `sessionQuery.observeSession()` 获取自身标题与轮次结果。实际 Agent 的运行／空闲状态优先，不能用“仍驻留”冒充“正在执行”。历史筛选包含当前未运行的会话，不表示全部成功。目录最多展示 256 条、优先运行中的 Agent；超限时显示已展示／总数，筛选计数只针对已展示条目。目录不可用不会影响主智能体流程和已有 Team／工作流记录的查看。

`npm run verify:execution-web` 使用临时 profile 和无密钥测试模型，在本地 DSH 页面验证泳道、嵌套队员、工具详情、失败状态、实时完成、会话跳转、窄屏及 light/dark/system 切换。设置 `DSH_VERIFY_AGGREGATE=1` 可验证聚合包；默认验证独立安装及多功能组合后清理未选功能。
