...
打开“启用桌面宠物”后，浏览器之外会显示原生 DeepSeek 鱼。“宠物样式”可在原有的“平面小鲸”和新增的“立体小鲸”之间实时切换，默认仍为平面小鲸。它会根据所有会话汇总为以下状态：

- **空闲**：持续播放轻微呼吸和 `Zzz` 漂浮的睡眠循环；鼠标接触或拖动时切换为蓄势待玩的“跃跃欲试”循环，鼠标离开后恢复睡眠；可选择空闲时不置顶。
- **任务中**：平面小鲸播放五帧专注游动；立体小鲸以二十四帧侧向强力游动、专注表情、气泡和短尾流表现任务进行中，不使用外部进度环。
- **需要确认**：平面小鲸播放五帧惊讶提醒和脉冲环；立体小鲸以二十四帧正面／侧面转头、抬鳍和问号表达等待确认，优先级最高且不使用外圈。
- **已完成**：仅为顶层任务短暂播放开心挥鳍／空间翻身与闪光动作。
- **任务受阻**：仅为顶层任务短暂播放疲惫／沮丧动作。

平面小鲸继续使用原有的五帧任务与空闲交互精灵图。立体小鲸的睡眠、鼠标交互、任务中、需要确认、已完成和任务受阻六组动画各有二十四张透明帧；镜头集中在正面、斜侧和侧面，不再以背面转身作为主要动作。鼠标交互是原地蓄势，任务中则使用更快的侧向游动和尾流，两者拥有独立姿态与节奏。动态来自视角、姿态、鳍、尾巴、眼睛、呼吸和表情变化，不靠静态图片抖动或外圈旋转。宠物可跨显示器自由拖动，拖动过程中允许越过桌面边缘；松开鼠标后，会优先完整吸附到重叠面积最大的显示器工作区，若停在显示器之间的空隙，则吸附到边缘距离最近的显示器。插件会按显示器保存归一化位置，并在分辨率、缩放、工作区或显示器连接状态变化后重新换算到可见区域。修改“启动位置”会清除拖动记录并恢复到所选角落。Windows 开启“减弱动画”后会自动使用每种任务状态和空闲交互阶段的代表静态帧。
...
| `petCharacter` | `classic` | 宠物样式：`classic`（平面小鲸）或 `multiview`（立体小鲸） |
