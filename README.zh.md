...
`dsh-enhanced-plugins` 是一个面向 DeepSeek Harness（DSH）的可安装增强插件合集。它将常用但彼此独立的扩展能力整合到同一个 bundle 中，让用户只需安装和维护一个包，就能完成 MCP 服务配置、模型输入能力声明、插件发现与安装、工作区文件引用，以及产品子智能体开关等操作。
